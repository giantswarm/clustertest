...
func (c *Cluster) GetRelease() (*releases.Release, error) {
	...
	if releaseVersion == ReleaseLatest {
		...
		for _, overrideApp := range c.appOverrides {
			logger.Log("Overriding Release app '%s' version '%s'", overrideApp.AppName, overrideApp.Version)
			releaseBuilder = releaseBuilder.WithApp(overrideApp.AppName, strings.TrimPrefix(overrideApp.Version, "v"), "", []string{})
		}
